...
class BenchmarkService(BenchmarkUseCases):
    ...
    async def generate_benchmark_data(self, lower_bound: float, upper_bound: float, metric: str, llm: str):
    ...
    async def simulate(self) -> List[BenchmarkEntity]:
        ...
        for llm in llms:
            for metric in metrics:
                metric_values = await self.generate_benchmark_data(metric.lower_bound, metric.upper_bound, metric.title, llm.name)
                ...
                if metric_values:
                    ...
                    if redis_client.redis.exists(f"{RedisKeys.METRIC_BENCHMARKS.value}:{metric.title}"):
                        redis_client.delete_key(f"{RedisKeys.METRIC_BENCHMARKS.value}:{metric.title}")

        logging.info(f"Benchmark simulation completed successfully")
    ...
    async def initiate_simulation(self):
        ...
        try:
            with redis_client.redis.lock(RedisKeys.RETRY_BENCHMARKS_LOCK.value, timeout=MAX_RETRIES * RETRY_DELAY, blocking=True, blocking_timeout=10) as lock:
                await self.simulate()
        except redis.exceptions.LockError as l:
            logging.error(f"Redis lock error: {l}")
